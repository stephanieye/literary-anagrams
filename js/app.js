$(()=>{
  ...
  const $currentAnagramBoard = $level.eq(levelcount).find($anagramboard);
  const $currentTile = $currentAnagramBoard.find($tile);
  const $answerboard = $('div.answerboard');
  const $currentAnswerBoard = $answerboard.eq(levelcount);
